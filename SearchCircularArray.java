...
public class SearchCircularArray {
    ...
    public static int searchCircularArray(int[] nums, int target)
    {
        ...
        while (left <= right)
        {
            ...
            //------------if right half nums[mid…right] is sorted and `mid` is not the key element
            if (nums[mid] <= nums[right])
            {
                //----------compare key with nums[mid] and nums[right] to know if it lies in nums[mid…right] or not
            ...
            //--------------if left half nums[left…mid] is sorted, and `mid` is not the key element
            else {
                //-------------compare key with nums[left] and nums[mid] to know if it lies in nums[left…mid] or not
